...
def get_random_keys(country_code, quantity=1):
    ...
    return [f"{k}#{KEY_NAME}" for k in selected]
...
def get_random_key_any_country():
    ...
    for country in countries:
        ...
        if keys:
            ...
            return f"{key}#{KEY_NAME}", country['name']
...
async def cmd_give(message: Message):
    ...
    try:
        ...
        if not keys:
            await message.answer(f"❌ Нет ключей для {country['name']}")
            return
...
"""
        ...
        await message.answer(f"✅ Ключ отправлен пользователю {target_user_id}")
...
"""
    ...
    await callback.answer("🎁 Бесплатный ключ выдан!")
...
"""
                ...
                await callback.message.answer("✅ Оплата подтверждена! Ключ выдан!", reply_markup=get_main_keyboard())
                await state.clear()
                await callback.answer()
                return
...
async def approve_order(callback: CallbackQuery):
    ...
    keys = get_random_keys(country_code, 1)
    if not keys:
        await callback.message.edit_text(f"❌ Нет ключей для {country}")
        await callback.answer()
        return
